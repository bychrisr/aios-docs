...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["aios-core (开源)"]
        direction TB
        EXT["管道中的 / 扩展点"]
        HOOK_RUNNER[".aios-core/hooks/ / pro-hook-runner.js"]
        GOTCHAS["gotchas-memory.js / (独立)"]
        CTX_LOADER["context-loader.js / (独立)"]
    end

    subgraph PRO_BOUNDARY["aios-pro (私有 — pro/)"]
        ...
        subgraph RETRIEVAL["第三层: 检索"]
            R1["memory-retriever.js / 渐进式披露"]
            R2["memory-loader.js / 管道集成"]
            R3["agent-memory-api.js / *recall, *remember"]
        end

        subgraph EVOLUTION["第四层: 演化"]
            E1["self-learner.js / 修正 + 启发式"]
            E2["rule-proposer.js / 自动演化"]
        end
    end
...
```mermaid
flowchart LR
    SS([SessionStart]) --> PIPELINE["管道加载 / CLAUDE.md + rules / + MEMORY.md (200 行)"]
    PIPELINE --> CHECK["isProAvailable()?"]
    CHECK -->|"否"| SKIP["跳过记忆加载 / (同当前)"]
    SKIP --> GREETING["没有 / 记忆统计的 Greeting"]

    PC([PreCompact]) --> HOOK["pro-hook-runner.js"]
    HOOK --> CHECK2["pro/ 存在?"]
    CHECK2 -->|"否"| EXIT["exit(0) / 什么都不发生"]
...
flowchart TD
    ...
    subgraph PRO_CAPTURE["aios-pro: pro/memory/"]
        subgraph EXTRACTORS["提取器"]
            EX1["会话摘要提取器 / 决策, 错误, 进度"]
            EX2["陷阱提取器 / 错误 3x → 自动陷阱"]
            EX3["修正提取器 / 修正 → 模式"]
        end

        subgraph CLASSIFIERS["分类器"]
            CL1["层级分类器 / session | daily | durable"]
            CL2["领域分类器 / episodic | semantic | / procedural | reflective"]
            CL3["范围分类器 / shared | agent-specific"]
        end

        EXTRACTORS --> CLASSIFIERS
        CLASSIFIERS --> PERSIST[("持久化到 / .aios/memories/")]
    end
...
flowchart LR
    subgraph NATIVE["Claude Code 原生 (始终工作)"]
        N1[".claude/memory/MEMORY.md / 最多 200 行 / 始终在 system prompt 中"]
        N2[".claude/agent-memory/[agent]/ / 每代理 MEMORY.md"]
    end

    subgraph PRO_MANAGED["pro/memory/ 管理 (当可用时)"]
        F1["共享记忆 / Markdown + frontmatter"]
        F2["代理记忆 / 每代理私有"]
        F3["index.json / 搜索索引"]
    end
...
flowchart TD
    ...
    subgraph PRO_EVOLUTION["pro/memory/ (演化)"]
        A1["self-learner.js / 提取规则/启发式"]
        A4["置信度评分器 / Score = evidence * recency"]
        P1["rule-proposer.js / 生成变更提案"]
    end
...
sequenceDiagram
    ...
    alt Pro 不可用
        ...
        SD->>SD: 读取转录上下文
        SD->>SD: 提取: 决策, 错误, / 修正, 进度
        SD->>SD: 分类: tier + sector + scope
        SD->>FS: 写入记忆文件 (.md)
        SD->>FS: 更新 index.json
        SD-->>CC: 注入摘要: / "5 个记忆已持久化。 / 使用 *recall 访问。"
    end
...
sequenceDiagram
    ...
    Note over UAP: 之前: Claude Code 已加载 / CLAUDE.md, rules, MEMORY.md
