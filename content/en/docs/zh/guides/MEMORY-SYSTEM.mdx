...
flowchart TB
    subgraph CLAUDE_NATIVE["第一层: Claude Code 原生"]
        direction TB
        CLAUDE_MD_G["CLAUDE.md 全局 / ~/.claude/CLAUDE.md"]
        CLAUDE_MD_W["CLAUDE.md 工作区 / Workspaces/.claude/CLAUDE.md"]
        CLAUDE_MD_P["CLAUDE.md 项目 / aios-core/.claude/CLAUDE.md"]
        RULES["Rules/*.md / 5 个规则文件"]
        AUTO_MEM["自动记忆 / ~/.claude/projects/.../memory/MEMORY.md / 前 200 行在 system prompt 中"]
        COMPOUND["复合分析 / memory/compound-analysis/ / 11 个文件 (9 内容 + manifest + summary)"]
        AGENT_MEM["代理记忆 / .claude/agent-memory/[agent]/MEMORY.md / 6 个有记忆的代理"]
        TRANSCRIPTS["会话转录 / ~/.claude/projects/.../[session].jsonl"]
        SESSIONS_IDX["会话索引 / sessions-index.json"]
    end

    subgraph AIOS_FRAMEWORK["第二层: AIOS 框架"]
        direction TB
        GOTCHAS["Gotchas 记忆 / .aios-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["上下文快照 / .aios-core/core/memory/context-snapshot.js"]
        FILE_EVO["文件演化追踪器 / .aios-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager / .aios-core/core/memory/timeline-manager.js"]
        CTX_LOADER["会话上下文加载器 / .aios-core/core/session/context-loader.js"]
        CTX_DETECT["上下文检测器 / .aios-core/core/session/context-detector.js"]
        SESS_STATE["会话状态 / .aios-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks / .aios-core/hooks/gemini/"]
        HOOKS_UNI["统一 Hooks / .aios-core/hooks/unified/"]
    end
    ...
    subgraph STORAGE_AIOS["AIOS 存储 (.aios/)"]
...
```mermaid
flowchart LR
    subgraph MEMORY_LAYER["记忆层 (.aios-core/core/memory/)"]
    ...
    subgraph SESSION_LAYER["会话层 (.aios-core/core/session/)"]
        CL["context-loader.js"]
        CD["context-detector.js"]
    end

    subgraph ORCHESTRATION["编排 (.aios-core/core/orchestration/)"]
        SS["session-state.js"]
    end

    subgraph ACTIVATION["激活管道 (.aios-core/development/scripts/)"]
        UAP["unified-activation-pipeline.js"]
        GB["greeting-builder.js"]
    end

    subgraph HOOKS["Hooks 系统 (.aios-core/hooks/)"]
...
flowchart TD
    ...
    subgraph LOAD["自动加载 — 固定顺序"]
        LOAD_GLOBAL["1. ~/.claude/CLAUDE.md / 用户全局指令"]
        LOAD_WORKSPACE["2. Workspaces/.claude/CLAUDE.md / 工作区级指令"]
        LOAD_PROJECT["3. aios-core/.claude/CLAUDE.md / 项目级指令"]
        LOAD_RULES["4. aios-core/.claude/rules/*.md / 5 个详细规则文件"]
        LOAD_MEMORY["5. ~/.claude/projects/.../memory/MEMORY.md / 自动记忆 (前 200 行)"]
    end
...
flowchart TD
    ...
    subgraph WRITE_FLOW["写入 (会话期间 — Claude 决定)"]
        W1["Claude 发现模式、 / 错误或洞察"] --> W2["Claude 决定 / 保存到记忆?"]
        W2 -->|"是"| W3["使用 Write/Edit 工具 / 更新 MEMORY.md"]
        W2 -->|"否"| W4["会话结束时 / 知识丢失"]
        W3 --> W5["可选地在 / memory/compound-analysis/ / 中创建主题文件"]
    end

    subgraph NO_FLUSH["会话结束"]
        E1([会话关闭]) --> E2["转录保存 / 为 .jsonl"]
        E2 --> E3["没有其他事情发生"]
...
flowchart LR
    ...
    SQUAD_AGENTS["Squad 代理 / (Claude Code Agents)"] -->|"frontmatter / memory: project"| AGENT_MEM

    NOTE["注意: 只有定义在 / .claude/agents/ 中的代理 / 使用此记忆。 / AIOS 代理 (.aios-core/ / development/agents/) / 不使用。"]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("持久化到: / .aios/gotchas.json / .aios/gotchas.md / .aios/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("持久化到: / .aios/snapshots/[id].json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("持久化到: / .aios/file-evolution/ / evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — 统一门面"]
        ...
        T4[("持久化到: / .aios/timeline/ / unified-timeline.json")]
    end
...
flowchart TD
    subgraph CTX_DETECT["context-detector.js"]
        ...
        CD2["有对话 / 历史?"]
        CD3["分析最近命令"]
        CD4["读取 .aios/session-state.json"]
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL6["返回: 类型, 消息, 前一代理, / 命令, 活动工作流"]
        CL7["updateSessionState()"]
        CL8["onTaskComplete()"]
        CL9[("持久化到: / .aios/session-state.json / TTL: 1 小时")]
    ...
    subgraph SESS_STATE["session-state.js — Story 11.5"]
        ...
        SS7[("持久化到: / docs/stories/ / .session-state.yaml / 无 TTL — 永久")]
...
sequenceDiagram
    ...
    Note over UAP: 管道之前, Claude Code / 已加载: CLAUDE.md (3 级), / rules/*.md, MEMORY.md (200 行)
    ...
    Note over CC: 没有 AIOS 记忆 / 自动加载 / 到 system prompt。 / 仅会话上下文 / 出现在 greeting 中。
```
...
flowchart TD
    ...
    subgraph SAVES["保存的内容"]
        S1[".aios/session-state.json / 当前代理, 命令, 工作流"]
        S2[".aios/error-tracking.json / 错误发生"]
        S3[".aios/gotchas.json / 新自动捕获的 gotcha"]
        S4[".aios/gotchas.json / 添加的手动 gotcha"]
        S5["docs/stories/.session-state.yaml / 更新的阶段"]
        S6["docs/stories/.session-state.yaml / Story 标记完成"]
        S7[".aios/snapshots/[id].json / 捕获的上下文"]
        S8[".aios/file-evolution/ / 记录的演化"]
    end
...
sequenceDiagram
    ...
    CL->>FS: 读取 .aios/session-state.json
    FS-->>CL: 当前状态 (或空)
    ...
    CL->>FS: 写入 .aios/session-state.json
    FS-->>CL: OK

    Note over Agent,FS: 工作流推断: / 36 个 task→state 映射 / 例: validate-next-story → / [workflow: story_development, / state: validated]
...
flowchart TD
    ...
    LOAD --> CHECK["存在于 / .session-state.yaml?"]

    CHECK -->|"否"| FRESH["新会话 / 无需恢复"]
    CHECK -->|"是"| DETECT["SessionState.detectCrash()"]

    DETECT --> THRESHOLD["最后活动 / > 30 分钟前?"]
    THRESHOLD -->|"否"| NORMAL["正常会话 / 从上次继续"]
    THRESHOLD -->|"是"| LAST_ACTION["最后操作是 / PAUSE, COMPLETED, / 或 ABORT?"]

    LAST_ACTION -->|"是"| NORMAL_END["正常结束 / 不是崩溃"]
    LAST_ACTION -->|"否"| CRASH["检测到崩溃"]

    CRASH --> RESUME_MENU["显示恢复选项"]

    subgraph RESUME_OPTIONS["恢复选项"]
        OPT1["CONTINUE / 从最后状态恢复"]
        OPT2["REVIEW / 显示进度摘要"]
        OPT3["RESTART / 重新开始当前 story"]
        OPT4["DISCARD / 丢弃会话"]
    end
...
flowchart TD
    subgraph ERROR_FLOW["自动捕获流程"]
        ...
        TRACK --> READ_ET["读取 .aios/error-tracking.json"]
        READ_ET --> NORMALIZE["规范化错误消息"]
        NORMALIZE --> HASH["生成消息哈希"]
        HASH --> INCREMENT["增加计数器"]
        INCREMENT --> CHECK["计数器 >= 3? / (repeatThreshold)"]

        CHECK -->|"否"| SAVE_ET["保存 error-tracking.json / 等待下次发生"]
        CHECK -->|"是"| WINDOW["在 24 小时 / 窗口内?"]

        WINDOW -->|"否"| RESET["重置计数器 / 重新开始计数"]
        WINDOW -->|"是"| AUTO_CAPTURE["_autoCaptureGotcha()"]

        AUTO_CAPTURE --> CREATE["创建 gotcha: / - id: gotcha-[hash] / - source: auto_detected / - category: 推断 / - severity: 推断"]

        CREATE --> SAVE_JSON["保存 .aios/gotchas.json"]
        SAVE_JSON --> SAVE_MD["生成 .aios/gotchas.md"]
    ...
    subgraph INJECTION_FLOW["任务注入流程"]
        ...
        QUERY --> MATCH["按以下条件过滤 gotchas: / - taskDesc 关键词 / - 相关文件 / - 相关类别"]
        MATCH --> FORMAT["formatForPrompt(gotchas)"]
        FORMAT --> INJECT["作为警告注入 / 任务上下文"]
    end
...
flowchart TD
    subgraph CAPTURE["捕获 — context-snapshot.js"]
        ...
        C2 --> C3["捕获 git 状态: / 分支, 提交, 脏文件"]
        C3 --> C4["记录: / storyId, agent, subtask, / 工作目录"]
        C4 --> C5["保存 .aios/snapshots/[id].json"]
        C5 --> C6["autoCleanup?"]
        C6 -->|"是"| C7["移除快照: / - > 50 个存在 / - > 7 天"]
    end

    subgraph RESTORE["恢复 — 恢复"]
        R1(["snapshot.restore(id)"]) --> R2["读取 .aios/snapshots/[id].json"]
        R2 --> R3["返回完整上下文"]
        R3 --> R4["代理可以从 / 上次位置恢复"]
    end

    subgraph LIST["列出 — 查询"]
        L1(["snapshot.list()"]) --> L2["列出 .aios/snapshots/ 中所有"]
        L2 --> L3["按时间戳排序"]
        L3 --> L4["返回元数据数组"]
    end
...
```mermaid
flowchart TD
    Q["Claude Code 中 / sessionStart?"] -->|"否 — claude: null"| GAP["Claude Code 中 / 没有原生 / session start 事件"]
    GAP --> WORKAROUND["当前变通方案: / 激活管道通过 Tier 3 / (SessionContextLoader) / 部分替代"]

    Q2["Claude Code 中 / sessionEnd?"] -->|"映射到 Stop"| STOP["Stop 事件存在 / 但 hook 未在 / .claude/settings.json / 中配置"]
    STOP --> NOT_WIRED["session-end.js 从未 / 在 Claude Code 中执行"]
