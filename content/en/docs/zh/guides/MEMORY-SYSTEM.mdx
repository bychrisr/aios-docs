...
flowchart TB
    subgraph CLAUDE_NATIVE["第一层: Claude Code 原生"]
        ...
        CLAUDE_MD_P["CLAUDE.md 项目 / aiox-core/.claude/CLAUDE.md"]
    ...
    subgraph AIOX_FRAMEWORK["第二层: AIOX 框架"]
        direction TB
        GOTCHAS["Gotchas 记忆 / .aiox-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["上下文快照 / .aiox-core/core/memory/context-snapshot.js"]
        FILE_EVO["文件演化追踪器 / .aiox-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager / .aiox-core/core/memory/timeline-manager.js"]
        CTX_LOADER["会话上下文加载器 / .aiox-core/core/session/context-loader.js"]
        CTX_DETECT["上下文检测器 / .aiox-core/core/session/context-detector.js"]
        SESS_STATE["会话状态 / .aiox-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks / .aiox-core/hooks/gemini/"]
        HOOKS_UNI["统一 Hooks / .aiox-core/hooks/unified/"]
    end
...
flowchart TD
    ...
    subgraph LOAD["自动加载 — 固定顺序"]
        ...
        LOAD_PROJECT["3. aiox-core/.claude/CLAUDE.md / 项目级指令"]
        LOAD_RULES["4. aiox-core/.claude/rules/*.md / 5 个详细规则文件"]
...
flowchart LR
    ...
    NOTE["注意: 只有定义在 / .claude/agents/ 中的代理 / 使用此记忆。 / AIOX 代理 (.aiox-core/ / development/agents/) / 不使用。"]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("持久化到: / .aiox/gotchas.json / .aiox/gotchas.md / .aiox/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("持久化到: / .aiox/snapshots/[id].json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("持久化到: / .aiox/file-evolution/ / evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — 统一门面"]
        ...
        T4[("持久化到: / .aiox/timeline/ / unified-timeline.json")]
    end
...
flowchart TD
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL8["onTaskComplete()"]
        CL9[("持久化到: / .aiox/session-state.json / TTL: 1 小时")]
...
sequenceDiagram
    ...
    Note over CC: 没有 AIOX 记忆 / 自动加载 / 到 system prompt。 / 仅会话上下文 / 出现在 greeting 中。
```
...
flowchart TD
    ...
    subgraph SAVES["保存的内容"]
        S1[".aiox/session-state.json / 当前代理, 命令, 工作流"]
        S2[".aiox/error-tracking.json / 错误发生"]
        S3[".aiox/gotchas.json / 新自动捕获的 gotcha"]
        S4[".aiox/gotchas.json / 添加的手动 gotcha"]
        S5["docs/stories/.session-state.yaml / 更新的阶段"]
        S6["docs/stories/.session-state.yaml / Story 标记完成"]
        S7[".aiox/snapshots/[id].json / 捕获的上下文"]
        S8[".aiox/file-evolution/ / 记录的演化"]
    end
