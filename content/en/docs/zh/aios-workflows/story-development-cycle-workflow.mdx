...
flowchart TD
    subgraph PHASE_1["阶段 1: 故事创建"]
        ...
        SM_CREATE["@sm: 创建下一个故事 / 任务: create-next-story"]
    end

    subgraph PHASE_2["阶段 2: 故事验证"]
        SM_CREATE --> PO_VALIDATE["@po: 验证故事 / 任务: validate-next-story / 10 个检查"]
        PO_VALIDATE --> VALID_DECISION[验证 OK?]
        VALID_DECISION -->|否| FEEDBACK_SM["向 SM 反馈 / 故事返回调整"]
        FEEDBACK_SM --> SM_CREATE
        VALID_DECISION -->|是| DEV_IMPLEMENT
    end

    subgraph PHASE_3["阶段 3: 实现"]
        DEV_IMPLEMENT["@dev: 实现故事 / 任务: dev-develop-story / + CodeRabbit 自我修复"]
    end

    subgraph PHASE_4["阶段 4: QA 审查"]
        DEV_IMPLEMENT --> QA_REVIEW["@qa: 审查 + 质量网关 / 任务: qa-gate"]
        QA_REVIEW --> QA_DECISION[质量网关 OK?]
        QA_DECISION -->|否| FEEDBACK_DEV["向开发反馈 / 修复检查清单"]
