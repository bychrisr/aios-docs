...
flowchart TB
    ...
    subgraph WORKFLOW["工作流序列"]
        S1["步骤 1: 提取故事上下文 / 提取故事 ID"]
        S2["步骤 2: 检查现有 / 检查现有工作树"]
        S3["步骤 3: 自动清理 / 清理过时的工作树"]
        S4["步骤 4: 创建工作树 / 创建隔离工作树"]
        S5["步骤 5: 切换上下文 / 切换到工作树"]
        S6["步骤 6: 显示摘要 / 显示摘要"]
    end
...
graph TB
    subgraph AIOS["AIOS 核心"]
        WF[auto-worktree.yaml / 工作流定义]
        TK[create-worktree.md / 任务定义]
    end

    subgraph INFRA["基础设施"]
        WM[worktree-manager.js / WorktreeManager 类]
        GIT[Git CLI / git worktree]
    end

    subgraph FS["文件系统"]
        WT[.aios/worktrees/[storyId]/ / 工作树目录]
        LOG[.aios/logs/merges/ / 合并审计日志]
    end
