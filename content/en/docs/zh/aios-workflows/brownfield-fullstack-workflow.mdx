...
flowchart TB
    subgraph CLASSIFICATION["1. 增强分类"]
        ...
        ANALYST_CLASSIFY["@analyst / 分类增强范围"]
    end

    ANALYST_CLASSIFY --> DECISION_SIZE[{"增强规模?"]}

    subgraph ROUTING["2. 按大小路由"]
        DECISION_SIZE -->|"单个故事 / (< 4小时)"| PM_STORY
        DECISION_SIZE -->|"小功能 / (1-3个故事)"| PM_EPIC
        DECISION_SIZE -->|"主要增强 / (多个史诗)"| ANALYST_DOCS
    ...
    subgraph DOCUMENTATION["3. 文档验证"]
        ANALYST_DOCS["@analyst / 验证现有文档"]
        ANALYST_DOCS --> DECISION_DOCS[{"文档充分?"]}
        DECISION_DOCS -->|否| ARCHITECT_DOCPROJ["@architect / document-project"]
        DECISION_DOCS -->|是| PM_PRD["@pm / 创建brownfield-prd.md"]
        ARCHITECT_DOCPROJ --> PM_PRD
    end

    subgraph PLANNING["4. 规划"]
        PM_PRD --> DECISION_ARCH[{"架构 / 变更?"]}
        DECISION_ARCH -->|是| ARCHITECT_ARCH["@architect / 创建architecture.md"]
        DECISION_ARCH -->|否| PO_VALIDATE
        ARCHITECT_ARCH --> PO_VALIDATE["@po / 验证工件 / (po-master-checklist)"]
    end

    subgraph VALIDATION["5. 验证和修正"]
        PO_VALIDATE --> DECISION_ISSUES[{"PO发现 / 问题?"]}
        DECISION_ISSUES -->|是| FIX_ISSUES["修复问题 / (相关代理)"]
        FIX_ISSUES --> PO_VALIDATE
        DECISION_ISSUES -->|否| PO_SHARD["@po / 分片文档"]
    end

    subgraph DEVELOPMENT["6. 开发周期"]
        PO_SHARD --> SM_STORY["@sm / 创建故事"]
        SM_STORY --> DECISION_STORY_TYPE[{"文档 / 类型?"]}
    ...
    subgraph COMPLETION["8. 完成"]
        ...
        DECISION_RETRO[{"回顾?"]}
        DECISION_RETRO -->|是| PO_RETRO["@po / 史诗回顾"]
