...
flowchart TB
    ...
    subgraph PREFLIGHT["飞行前检查"]
        PF1["story_exists / 验证/创建目录"]
        PF2["no_existing_spec / 避免覆盖"]
        PF3["agents_available / 验证配置"]
    end
    ...
    subgraph PHASE2["阶段 2: 评估复杂性"]
        direction TB
        A1["@architect (Aria)"]
        A2["评估 5 个维度: / 范围、集成、基础设施、 / 知识、风险"]
        A3["complexity.json / 简单 | 标准 | 复杂"]
        A1 --> A2 --> A3
    end
    ...
    subgraph PHASE5["阶段 5: 批评规范"]
        direction TB
        C1["@qa (Quinn)"]
        C2["评估 5 个维度: / 准确性、完整性、 / 一致性、可行性、 / 对齐"]
        C3["判决"]
        C4["已批准"]
        C5["需要修订"]
        C6["阻止"]
    ...
    subgraph PHASE6["阶段 6: 创建计划"]
        direction TB
        P1["@architect (Aria)"]
        P2["生成 implementation.yaml / 原子子任务"]
        P3["plan.json"]
        P1 --> P2 --> P3
    end
...
flowchart TB
    ...
    CHECK1 -->|否| CHECK2["平均 / 评分 >= 4.0?"]

    CHECK2 -->|是| CHECK3["所有维度 / >=3?"]
    CHECK3 -->|是| APPROVED["已批准"]
    CHECK3 -->|否| NEEDS["需要修订"]

    CHECK2 -->|否| CHECK4["平均 / 评分 >= 3.0?"]
    CHECK4 -->|是| NEEDS
    CHECK4 -->|否| BLOCKED
...
graph LR
    subgraph AGENTS["Spec 流水线代理"]
        PM["@pm / Morgan / 产品经理"]
        AR["@architect / Aria / 架构师"]
        AN["@analyst / Atlas / 业务分析师"]
        QA["@qa / Quinn / 测试架构师"]
    end

    PM --> |"阶段 1, 4, 5b"| G1["收集 / 编写规范 / 修订"]
    AR --> |"阶段 2, 6"| G2["评估 / 计划"]
    AN --> |"阶段 3"| G3["研究"]
    QA --> |"阶段 5, 5c"| G4["批评"]
...
```mermaid
flowchart TB
    D1["source === 'simple' / 或 / overrideComplexity === 'SIMPLE'?"]
    D1 -->|是| SKIP["跳转到规范 / (假设简单)"]
    D1 -->|否| RUN["执行评估"]
```
...
```mermaid
flowchart TB
    D2["complexity.result === 'SIMPLE' / 或 / 没有外部依赖?"]
    D2 -->|是| SKIP["跳过研究 / 生成最小 research.json"]
    D2 -->|否| RUN["执行研究"]
```
...
```mermaid
flowchart TB
    D4["complexity === 'COMPLEX' / 或 / verdict === 'NEEDS_REVISION'?"]
    D4 -->|是| RUN["执行修订 / (阶段 5b)"]
    D4 -->|否| SKIP["跳过修订"]
```
