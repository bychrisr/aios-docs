...
flowchart TD
    subgraph FASE_1_3["阶段1-3：数据收集"]
        A[("启动：棕地项目发现")] --> B["@architect / 系统文档"]
        B --> C[项目有 / 数据库?]
        C -->|是| D["@data-engineer / Schema + 审计"]
        C -->|否| E["@ux-design-expert / 前端规范"]
        D --> E
    end

    subgraph FASE_4["阶段4：初步整合"]
        E --> F["@architect / 整合DRAFT"]
    end

    subgraph FASE_5_7["阶段5-7：专家验证"]
        F --> G["@data-engineer / 验证DB部分"]
        G --> H["@ux-design-expert / 验证UX部分"]
        H --> I["@qa / 质量关卡审查"]
        I --> J[QA关卡?]
        J -->|需要修复| K["返工： / 应用修正到DRAFT"]
        K --> F
    end

    subgraph FASE_8_9["阶段8-9：最终报告"]
        J -->|已批准| L["@architect / 最终评估"]
        L --> M["@analyst / 执行报告"]
    end
...
mindmap
  root((棕地项目 / 发现))
    收集
      architect["@architect / Aria (愿景家) / 系统和整合"]
      data-engineer["@data-engineer / Dara (智者) / 数据库"]
      ux-design-expert["@ux-design-expert / Uma (同情者) / 前端/UX"]
    验证
      data-engineer2["@data-engineer / 审查DB"]
      ux-design-expert2["@ux-design-expert / 审查UX"]
      qa["@qa / Quinn (守护者) / 质量关卡"]
    完成
      architect2["@architect / 最终评估"]
      analyst["@analyst / Atlas (解码器) / 执行报告"]
      pm["@pm / Morgan (战略家) / 史诗和故事"]
```
...
flowchart TD
    Q1[项目有 / 数据库?]
    Q1 -->|是| A1[执行阶段2： / @data-engineer审计DB]
    Q1 -->|否| A2[跳过阶段2： / 直接到阶段3]
    A1 --> B[阶段3：前端]
    A2 --> B
```
...
flowchart TD
    Q2[QA关卡状态?]
    Q2 -->|已批准| A3[继续阶段8： / 最终评估]
    Q2 -->|需要修复| A4[返回阶段4： / 返工DRAFT]
    A4 --> A5[整合QA反馈]
    A5 --> A6[重新提交验证]
    A6 --> Q2
```
...
flowchart TD
    Q3[史诗范围]
    Q3 -->|1-3个故事| A7[使用brownfield-create-epic / 简单工作流]
    Q3 -->|4+个故事| A8[考虑完整PRD / 扩展工作流]
