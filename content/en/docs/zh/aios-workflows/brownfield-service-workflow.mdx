...
graph TD
    subgraph "阶段1：规划"
        A[启动：服务增强] --> B["@architect：分析现有服务 / (document-project)"]
        B --> C["@pm：创建PRD / (brownfield-prd-tmpl)"]
        C --> D["@architect：创建架构 / (brownfield-architecture-tmpl)"]
    end

    subgraph "阶段2：验证"
        D --> E["@po：验证工件 / (po-master-checklist)"]
        E --> F[PO发现 / 问题?]
        F -->|是| G[返回相关代理 / 进行修正]
        G --> E
        F -->|否| H["@po：分片文档 / (shard-doc)"]
    end

    subgraph "阶段3：开发周期"
        H --> I["@sm：创建故事 / (create-next-story)"]
        I --> J[审查故事 / 草稿?]
        J -->|是| K["@analyst/@pm：审查和 / 批准故事"]
        J -->|否| L["@dev：实现故事 / (develop-story)"]
        K --> L
        L --> M[QA审查?]
        M -->|是| N["@qa：审查实现 / (review-story)"]
        M -->|否| O[更多故事?]
        N --> P[QA发现 / 问题?]
        P -->|是| Q["@dev：修复QA反馈"]
        P -->|否| O
    ...
    subgraph "阶段4：完成"
        R -->|是| S["@po：史诗回顾 / (epic-retrospective)"]
        R -->|否| T[项目完成]
        S --> T
    end
...
graph LR
    subgraph "规划代理"
        Architect["@architect / Aria / 整体系统架构师"]
        PM["@pm / Morgan / 产品经理"]
        Analyst["@analyst / Atlas / 业务分析师"]
    end
