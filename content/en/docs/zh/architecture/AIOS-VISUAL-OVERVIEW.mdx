...
flowchart TB
    subgraph TASK_ANATOMY["任务解剖"]
        ...
        PRE["前置条件"]
        POST["后置条件"]
        EXEC["执行器类型: / 代理 | 工作器 | 克隆 | 人类"]
        AC["验收标准"]
    end

    TASK_ANATOMY --> VALIDATE["任务已验证?"]
    VALIDATE -->|"是"| EXECUTE["任何执行器 / 都能保证执行"]
    VALIDATE -->|"否"| REJECT["拒绝: / 先完善任务"]
...
flowchart TB
    subgraph EXECUTORS["AIOS 的 4 种执行器"]
        ...
        subgraph AGENT_BOX["代理 — 生成式 AI"]
            AGENT_DESC["使用 LLM 处理创意、 / 分析或生成任务"]
            AGENT_EX["例如: 分析简报、生成文案、 / 选择模板、代码审查"]
        end

        subgraph WORKER_BOX["工作器 — 确定性脚本"]
            WORKER_DESC["使用预定义逻辑 / 转换数据的代码/脚本"]
            WORKER_EX["例如: 加载 JSON 配置、 / 验证 HTML、导出 PNG、 / 计算间距"]
        end

        subgraph CLONE_BOX["克隆 — AI + 领域启发式"]
            CLONE_DESC["增强了特定人物的公理 / 和方法论的 AI 代理"]
            CLONE_EX["例如: 验证原子设计 (Brad Frost)、 / 审核文案 (Alex Hormozi)、 / 评估用户体验 (Jakob Nielsen)"]
        end

        subgraph HUMAN_BOX["人类 — 人类操作员"]
            HUMAN_DESC["执行需要主观判断 / 的任务的真人"]
            HUMAN_EX["例如: 批准 $10k 活动、 / 法律合规、战略决策、 / 关键边缘案例"]
        end
    end
...
flowchart TD
    ...
    Q3 -->|"是"| Q5["关键决策 / 有法律/财务 / 影响?"]
    Q3 -->|"否"| Q6["需要 / 特定 / 方法论?"]

    Q4 -->|"是 (非 AI)"| WORKER1["工作器 / 带 API"]
    Q4 -->|"否"| WORKER2["工作器 / 脚本"]

    Q5 -->|"是"| HUMAN["人类 / 人类操作员"]
    Q5 -->|"否"| Q6

    Q6 -->|"是"| CLONE["克隆 / AI + 启发式"]
    Q6 -->|"否"| AGENT["代理 / 生成式 AI"]
...
flowchart LR
    subgraph SUBSTITUTION["执行器替换规则"]
        ...
        R1["代理 -> 工作器 / 当任务变得 / 数据确定性"]
        R2["人类 -> 代理 / 当自动化达到 / 可接受精度"]
        R3["代理 -> 克隆 / 当特定方法论 / 提高质量"]
        R4["克隆 -> 代理 / 当方法论对 / 结果不关键"]
    end
...
flowchart LR
    subgraph H1["代理 + 工作器 (回退)"]
        A1["代理尝试"] -->|"失败"| W1["工作器接管 / (简单规则)"]
    end

    subgraph H2["代理 + 人类 (审核)"]
        A2["代理处理 / 100% 的量"] -->|"得分 < 80%"| HU1["人类审核 / 关键案例"]
    end

    subgraph H3["克隆 + 代理 (验证)"]
        A3["代理创建 / (创意)"] --> C1["克隆验证 / (方法论)"]
        C1 -->|"无效"| A3
    end
...
flowchart TB
    subgraph WS["公司动态工作空间"]
        ...
        subgraph FRAMEWORK[".aios-core/ — 框架 (不可变)"]
            AGENTS_CORE["11 个核心代理 / dev, qa, architect, pm, / po, sm, analyst, devops, / data-engineer, ux, aios-master"]
            TASKS_CORE["45+ 可执行 / 任务"]
            WORKFLOWS["可编排 / 工作流"]
            CONSTITUTION["宪法 / 不可协商原则"]
        end

        subgraph SQUADS["squads/ — 模块化团队"]
            SQ1["营销小队 / 文案, 设计师, / 流量经理"]
            SQ2["数据小队 / 数据工程师, 分析师, / ETL 专家"]
            SQ3["领域小队 / 公司业务 / 专业代理"]
        end

        subgraph MINDS["认知克隆"]
            M1["Pedro Valerio / 流程 &amp; 系统"]
            M2["Brad Frost / 原子设计"]
            M3["Alex Hormozi / 文案写作"]
            M4["自定义克隆 / 公司专家"]
        end
        ...
        subgraph CONFIG["按公司配置"]
            ...
            TS["技术栈"]
            ST["源码树"]
            IDE["IDE 规则 / (Claude Code, Cursor, / Copilot, Gemini)"]
        end
    end
...
flowchart LR
    ...
    WIZARD --> CHOOSE_SQUADS["选择小队 / (初始小队)"]
    WIZARD --> CHOOSE_IDE["选择 IDE / (Claude Code, Cursor...)"]
    WIZARD --> CHOOSE_CONFIG["配置 / 偏好"]

    CHOOSE_SQUADS --> SQ_OPS["HybridOps / (Pedro Valerio)"]
    CHOOSE_SQUADS --> SQ_CUSTOM["自定义小队 / (公司领域)"]
    CHOOSE_SQUADS --> SQ_MARKET["市场 / (现成小队)"]
    ...
    SQ_OPS --> READY(["工作空间就绪! / 人类 + AI / 协同工作"])
    SQ_CUSTOM --> READY
    SQ_MARKET --> READY
    IDE1 --> READY
    IDE2 --> READY
...
flowchart TB
    subgraph COLLAB["小队 + 人类协作"]
        ...
        subgraph PLANNING["阶段 1: 规划"]
            H_PM["人类 PM"] <-->|"完善需求"| AI_PM["@pm (Morgan) / 代理"]
            H_PM <-->|"验证架构"| AI_ARCH["@architect (Aria) / 代理"]
            AI_PM --> PRD["完整 PRD"]
            AI_ARCH --> ARCH_DOC["架构 / 文档"]
        end

        subgraph EXECUTION["阶段 2: 开发"]
            AI_SM["@sm (River) / 代理"] -->|"创建故事"| STORIES["超详细 / 故事"]
            STORIES --> AI_DEV["@dev (Dex) / 代理"]
            AI_DEV -->|"实现"| CODE["代码"]
            CODE --> AI_QA["@qa (Quinn) / 代理"]
            H_DEV["人类开发者"] <-->|"结对编程"| AI_DEV
            H_DEV <-->|"代码审查"| AI_QA
        end

        subgraph REVIEW["阶段 3: 质量门禁"]
            AI_QA -->|"通过"| AI_DEVOPS["@devops (Gage) / 代理"]
            AI_QA -->|"失败"| AI_DEV
...
```mermaid
flowchart TD
    IDEA(["创意 / 需求"]) --> ANALYST["@analyst / 研究 &amp; 分析"]
    ANALYST --> PM["@pm (Morgan) / 创建 PRD"]
    PM --> ARCH["@architect (Aria) / 架构设计"]

    ARCH --> SM["@sm (River) / 创建故事"]
    SM --> PO["@po (Pax) / 验证故事? / 10点检查表"]

    PO -->|"通过 >= 7/10"| DEV["@dev (Dex) / 实现"]
    PO -->|"不通过 < 7/10"| SM

    DEV --> CODERABBIT["CodeRabbit / 自愈 / 最多 2 次迭代"]
    CODERABBIT -->|"严重"| DEV
    CODERABBIT -->|"通过"| QA["@qa (Quinn) / 质量门禁 / 7 项检查"]

    QA -->|"通过"| DEVOPS["@devops (Gage) / 推送 + PR"]
    QA -->|"顾虑"| DEVOPS
    QA -->|"失败"| DEV
    ...
    subgraph TASKS_FLOW["每一步都是一个任务"]
        direction LR
        NOTE["上面的每个框都是一个任务。 / 每个任务定义输入、输出、 / 前置/后置条件。 / 执行器 (代理) 只是 / 默认类型 — 也可以是 / 工作器、克隆或人类。"]
    end
...
flowchart LR
    ...
    subgraph AIOS_WAY["Synkra AIOS (Task-First)"]
        direction TB
        A1["带验收标准的 / 已验证任务"] --> A2["决策树选择 / 理想执行器"]
        A2 --> A3["每次转换 / 都有质量门禁"]
        A3 --> A4["结果: 可靠 / 且可扩展"]
    end
