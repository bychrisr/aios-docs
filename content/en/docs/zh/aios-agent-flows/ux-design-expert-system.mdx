...
flowchart TB
    subgraph PHASE1["第1阶段: UX研究"]
        R1["*research / 用户研究"]
        R2["*wireframe / 创建线框"]
        R3["*generate-ui-prompt / v0/Lovable提示"]
        R4["*create-front-end-spec / 前端规范"]
    end

    subgraph PHASE2["第2阶段: 设计系统审计"]
        A1["*audit [path] / 扫描冗余"]
        A2["*consolidate / 智能聚类"]
        A3["*shock-report / 视觉HTML报告"]
    end
    ...
    subgraph PHASE3["第3阶段: 设计令牌与设置"]
        ...
        T2["*setup / 初始化DS"]
        T3["*migrate / 迁移策略"]
        T4["*upgrade-tailwind / 升级Tailwind v4"]
        T5["*bootstrap-shadcn / 安装Shadcn/Radix"]
        T6["*audit-tailwind-config / 验证Tailwind配置"]
        T7["*export-dtcg / 导出W3C DTCG令牌"]
    end

    subgraph PHASE4["第4阶段: 组件构建"]
        B1["*build [component] / 构建原子"]
        B2["*compose [molecule] / 组成分子"]
        B3["*extend [component] / 添加变体"]
    end

    subgraph PHASE5["第5阶段: 质量与文档"]
        Q1["*document / 模式库"]
        Q2["*a11y-check / WCAG审计"]
        Q3["*calculate-roi / 计算ROI"]
    end
