...
flowchart TB
    ...
    subgraph DESIGN["设计阶段"]
        BLUEPRINT["蓝图 / .designs/[name]-design.yaml"]
        ANALYSIS["领域分析 / - 实体 / - 工作流 / - 集成"]
        RECOMMEND["建议 / - 代理 / - 任务 / - 置信度分数"]
    end
    ...
    subgraph CREATE["创建阶段"]
        TEMPLATE["模板选择 / - basic / - etl / - agent-only"]
        VERSION["版本选择 / - v1（遗留） / - v2（编排）"]
        GENERATE["生成结构 / - squad.yaml / - agents/ / - tasks/ / - workflows/"]
    end
    ...
    subgraph VALIDATE["验证阶段"]
        SCHEMA["Schema 验证 / squad-schema.json"]
        STRUCTURE["结构检查 / - tasks/ / - agents/ / - 引用的文件"]
        TASK_FMT["任务格式 / TASK-FORMAT-V1"]
        AGENT_FMT["代理格式 / YAML 结构"]
    end
    ...
    subgraph DISTRIBUTE["分发"]
        LOCAL["本地 / ./squads/"]
        PUBLIC["公共 / github.com/SynkraAI/aios-squads"]
        MARKET["市场 / api.synkra.dev/squads"]
    end
...
flowchart TB
    ...
    subgraph V2_PATH["v2 - 编排 + 技能"]
        V2_YAML["squad.yaml v2 / - 编排配置 / - 技能配置 / - 元数据"]
        V2_WF["workflows/main-workflow.yaml / - 阶段定义 / - 错误处理 / - 超时配置"]
        V2_AGENT["agents/ with skill_dispatch / - auto_inject 技能"]
    end

    subgraph V1_PATH["v1 - 遗留结构"]
        V1_YAML["squad.yaml v1 / - 基本清单 / - 组件列表"]
        V1_EMPTY["空 workflows/ / （无编排）"]
        V1_AGENT["agents/ 基本 / （无技能）"]
    end
    ...
    subgraph COMMON["通用结构"]
        CONFIG["config/ / - coding-standards.md / - tech-stack.md / - source-tree.md"]
        TASKS["tasks/ / - example-task.md"]
        DIRS["空目录： / checklists/ / templates/ / tools/ / scripts/ / data/"]
    end
...
flowchart TB
    subgraph INPUT["输入阶段"]
        DOCS["文档文件 / - PRD / - 规范 / - 需求"]
        VERBAL["口头描述 / （交互式）"]
        DOMAIN["领域提示 / --domain 标志"]
    end

    INPUT --> NORMALIZE["1. 输入规范化 / - 解析 markdown/yaml/json / - 提取文本内容 / - 合并来源"]

    NORMALIZE --> ANALYZE["2. 领域分析"]

    subgraph ANALYZE["分析管道"]
        ENTITY["实体提取 / - 名词/专有名词 / - 领域术语 / - 组概念"]
        WORKFLOW["工作流检测 / - 动作动词 / - 顺序流程 / - I/O 模式"]
        INTEGRATION["集成映射 / - 外部系统 / - API/服务"]
        STAKE["利益相关者 ID / - 用户角色 / - 人物画像"]
    end

    ANALYZE --> RECOMMEND["3. 建议引擎"]

    subgraph RECOMMEND["建议"]
        AGENTS["代理生成 / - 工作流角色 / - 步骤命令 / - 置信度计算"]
        TASKS_R["任务生成 / - TASK-FORMAT-V1 / - 输入的 Entrada / - 输出的 Saida"]
        DEDUP["去重 / - 合并 >70% 重叠"]
    end
    ...
    REVIEW --> BLUEPRINT["蓝图输出 / .designs/[name]-design.yaml"]
...
flowchart TB
    ...
    START --> RESOLVE["1. 解析小队路径 / ./squads/[name]/ 或完整路径"]

    RESOLVE --> MANIFEST["2. 清单验证"]

    subgraph MANIFEST["清单检查"]
        FIND["查找清单 / squad.yaml 或 config.yaml"]
        PARSE["解析 YAML"]
        SCHEMA["验证 vs JSON Schema / - name（kebab-case） / - version（semver） / - components"]
    end

    MANIFEST --> STRUCTURE["3. 结构验证"]

    subgraph STRUCTURE["结构检查"]
        DIRS["检查目录 / - tasks/（必需） / - agents/（必需）"]
        FILES["检查引用文件 / - components.tasks 存在？ / - components.agents 存在？"]
    end

    STRUCTURE --> TASKS_V["4. 任务验证"]

    subgraph TASKS_V["任务格式检查"]
        T_FIELDS["必需字段： / - task / - responsavel / - responsavel_type / - atomic_layer / - Entrada / - Saida / - Checklist"]
        T_NAMING["命名约定 / kebab-case"]
    end

    TASKS_V --> AGENTS_V["5. 代理验证"]

    subgraph AGENTS_V["代理检查"]
        A_FORMAT["代理格式 / - YAML frontmatter / - Markdown 标题"]
        A_NAMING["命名约定 / kebab-case"]
    end
