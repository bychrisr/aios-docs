...
flowchart TB
    subgraph INPUTS["输入"]
        PRD["PRD/史诗 / （来自 @pm）"]
        BACKLOG["已优先排序的待办 / （来自 @po）"]
        ARCH["架构 / （docs/architecture/）"]
    end
    ...
    subgraph SM_AGENT["@sm (River) - Scrum Master"]
        ...
        subgraph COMMANDS["可用命令"]
            DRAFT["*draft / 创建下一个故事"]
            CHECKLIST["*story-checklist / 验证故事草稿"]
    ...
    VALIDATE --> STORY_FILE["docs/stories/epic-X/ / STORY-X.Y.md"]
    VALIDATE --> CLICKUP["ClickUp 任务 / （自动同步）"]
    ...
    subgraph COLLABORATION["协作"]
        DEV_AGENT["@dev (Dex) / 接收故事"]
        PO_AGENT["@po (Pax) / 验证故事"]
        DEVOPS_AGENT["@github-devops (Gage) / 完成后 Push/PR"]
    end
...
```mermaid
flowchart TD
    A[开始：故事开发周期] --> B["@sm：创建下一个故事 / *draft"]
    B --> C["@po：验证故事 - 10 项检查 / *validate-story-draft"]

    C --> D[验证 OK？]
    D -->|否| E[SM 反馈]
    E --> B
    D -->|是| F["@dev：实现故事 / *develop"]

    F --> G["@qa：审查 + 质量门禁 / *review-story"]
...
flowchart LR
    subgraph SM_SCOPE["@sm - 本地范围"]
        CREATE_BRANCH["git checkout -b / feature/X.Y-story-name"]
        LIST_BRANCH["git branch / 列出分支"]
        SWITCH_BRANCH["git checkout / 切换分支"]
        DELETE_LOCAL["git branch -d / 删除本地"]
        MERGE_LOCAL["git merge / 本地合并"]
    end

    subgraph DEVOPS_SCOPE["@github-devops - 远程范围"]
        PUSH["git push / 发送到 origin"]
        CREATE_PR["gh pr create / 创建 Pull Request"]
        DELETE_REMOTE["git push origin --delete / 删除远程分支"]
    end

    SM_SCOPE -->|"故事完成 / 通知"| DEVOPS_SCOPE
...
flowchart TB
    subgraph UPSTREAM["上游 - 提供给 @sm"]
        PM_UP["@pm (Morgan) / 创建史诗结构"]
        PO_UP["@po (Pax) / 优先排序待办"]
        ANALYST_UP["@analyst (Sage) / 研究和见解"]
    end

    SM_CENTRAL["@sm (River) / Scrum Master"]

    subgraph DOWNSTREAM["下游 - 从 @sm 接收"]
        DEV_DOWN["@dev (Dex) / 实现故事"]
        PO_DOWN["@po (Pax) / 验证故事"]
        QA_DOWN["@qa (Quinn) / 风险分析"]
    end

    subgraph LATERAL["横向 - 与 @sm 协调"]
        DEVOPS_LAT["@github-devops (Gage) / Push/PR 工作流"]
    end
