...
flowchart TB
    ...
    subgraph PM_CORE["@pm (Morgan) - 核心"]
        CREATE_PRD["*create-prd / 创建绿地 PRD"]
        CREATE_BF_PRD["*create-brownfield-prd / 创建棕地 PRD"]
        CREATE_EPIC["*create-epic / 创建史诗"]
        CREATE_STORY["*create-story / 创建故事"]
        RESEARCH_CMD["*research / 深度研究"]
        CORRECT["*correct-course / 纠正偏差"]
        SHARD["*shard-prd / 拆分文档"]
    end
...
```mermaid
flowchart TD
    START[开始 *create-prd] --> CHECK_BRIEF[项目简介 / 可用？]

    CHECK_BRIEF -->|是| LOAD_BRIEF[加载简介]
    CHECK_BRIEF -->|否| RECOMMEND[建议创建简介 / 或收集信息]

    LOAD_BRIEF --> GOALS[1. 目标和背景]
    RECOMMEND --> GOALS

    GOALS --> REQUIREMENTS[2. 需求 / FR + NFR]
    REQUIREMENTS --> UI_GOALS[PRD 有 / UI/UX 需求？]

    UI_GOALS -->|是| UI_SECTION[3. UI 设计目标]
    UI_GOALS -->|否| TECH
    UI_SECTION --> TECH[4. 技术假设]

    TECH --> EPIC_LIST[5. 史诗列表 / 结构审批]
    EPIC_LIST --> EPIC_DETAILS[6. 史诗详情 / 故事 + AC]

    EPIC_DETAILS --> CHECKLIST[7. 执行 pm-checklist]
    CHECKLIST --> NEXT_STEPS[8. 下一步 / @architect 和 @ux-expert 提示]
...
```mermaid
flowchart TD
    START[开始 *create-brownfield-prd] --> ASSESS[评估增强 / 复杂度]

    ASSESS -->|简单 / 1-2 会话| RECOMMEND_SIMPLE["建议： / *create-epic 或 / *create-story"]
    ASSESS -->|重大 / 多个故事| CONTINUE[继续棕地 PRD]

    CONTINUE --> CHECK_DOC[document-project / 已执行？]

    CHECK_DOC -->|是| LOAD_DOC[加载现有分析]
    CHECK_DOC -->|否| ANALYZE[分析项目 / 或建议 document-project]

    LOAD_DOC --> OVERVIEW[1. 现有项目概览]
    ANALYZE --> OVERVIEW

    OVERVIEW --> SCOPE[2. 增强范围定义]
    SCOPE --> REQUIREMENTS[3. 需求 / FR + NFR + CR]

    REQUIREMENTS --> UI[增强 / 包含 UI？]
    UI -->|是| UI_ENHANCE[4. UI 增强目标]
    UI -->|否| TECH
    UI_ENHANCE --> TECH[5. 技术约束]

    TECH --> EPIC_STRUCT[6. 史诗结构]
    EPIC_STRUCT --> STORIES[7. 带 / 集成验证的故事]

    STORIES --> OUTPUT[docs/prd.md / 棕地]
...
```mermaid
flowchart TD
    TRIGGER[检测到变更] --> SETUP[1. 初始设置 / 交互模式]
    SETUP --> CHECKLIST[2. 执行检查清单 / 部分 1-4]
    CHECKLIST --> DRAFT[3. 起草变更 / 提案]
    DRAFT --> PROPOSAL[4. 生成冲刺 / 变更提案]
    PROPOSAL --> FINALIZE[5. 最终确定和 / 确定下一步]
    ...
    FINALIZE --> HANDOFF_DECISION[变更 / 性质？]
    HANDOFF_DECISION -->|可实施| IMPLEMENT[通过 / @po/@sm 实施]
    HANDOFF_DECISION -->|需要重新规划| ESCALATE[升级到 / @pm/@architect]
```
...
flowchart TB
    subgraph PM_BOX["@pm (Morgan) - 产品经理"]
        PM_DESC["创建 PRD、史诗、战略研究 / 纠正偏差"]
        PM_CMDS["命令： / *create-prd、*create-epic / *research、*correct-course"]
    end

    subgraph PO_BOX["@po (Pax) - 产品负责人"]
        PO_DESC["管理待办、验证故事 / 优先排序工作"]
        PO_CMDS["命令： / *backlog-*、*sync-story / *validate-story-draft"]
    end

    subgraph SM_BOX["@sm (River) - Scrum Master"]
        SM_DESC["创建详细故事 / 协调冲刺"]
        SM_CMDS["命令： / *create-next-story / 冲刺规划"]
    end

    subgraph ARCHITECT_BOX["@architect (Aria) - 架构师"]
        ARCH_DESC["架构设计 / 技术决策"]
        ARCH_CMDS["命令： / *create-architecture / *document-project"]
    end

    subgraph ANALYST_BOX["@analyst (Atlas) - 分析师"]
        ANALYST_DESC["市场研究 / 见解和数据"]
        ANALYST_CMDS["命令： / *brainstorm / *analyze"]
    end

    PM_BOX -->|"PRD 已批准 / 交接给架构"| ARCHITECT_BOX
    PM_BOX -->|"史诗已创建 / 委派故事创建"| SM_BOX
    PM_BOX -->|"接收见解"| ANALYST_BOX
    PM_BOX -->|"提供战略 / 方向"| PO_BOX

    PO_BOX -->|"验证故事 / 优先排序待办"| SM_BOX
    ANALYST_BOX -->|"为 PRD 提供 / 研究"| PM_BOX
...
flowchart LR
    ...
    PHASE_1_9 -->|"评估完成 / docs/prd/technical-debt-assessment.md"| PM
    PM --> EPIC
    EPIC --> STORY
    STORY -->|"故事就绪 / docs/stories/story-*.md"| DEV["@dev"]
