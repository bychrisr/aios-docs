...
flowchart TB
    subgraph PM_BOX["产品经理(Morgan)"]
        PM_DESC["创建PRD、epic、战略研究 / 纠正偏航"]
    end

    subgraph PO_BOX["产品所有者(Pax)"]
        PO_DESC["管理积压、验证story / 优先化工作"]
    end

    subgraph SM_BOX["Scrum Master(River)"]
        SM_DESC["创建详细story / 协调sprint"]
    end
    ...
    subgraph ANALYST_BOX["分析师(Atlas)"]
        ANALYST_DESC["市场研究 / 洞察和数据"]
    end

    PM_BOX -->|"批准的PRD / 架构交接"| ARCHITECT_BOX
    PM_BOX -->|"Epic创建 / 委派story创建"| SM_BOX
    PM_BOX -->|"接收洞察"| ANALYST_BOX
    PM_BOX -->|"提供战略方向"| PO_BOX

    PO_BOX -->|"验证story / 优先化积压"| SM_BOX
