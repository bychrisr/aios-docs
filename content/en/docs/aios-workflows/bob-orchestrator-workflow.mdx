...
flowchart TB
    subgraph STARTUP["1. STARTUP"]
        A1[("@pm ativado")] --> A2["resolveConfig() / L1→L2→L3→L4→L5"]
        A2 --> A3{"user_profile / == 'bob'?"}
        A3 -->|Não| PM_TRAD["PM Tradicional / STOP"]
        A3 -->|Sim| A4["CLEANUP automático / (locks, sessions, snapshots)"]
    ...
    subgraph TREE["3. DECISION TREE"]
        S4 --> T1{"Tem config?"}
        T1 -->|Não| PA["PATH A: / ONBOARDING"]
        T1 -->|Sim| T2{"Tem docs AIOS?"}
        T2 -->|Não| PB["PATH B: / BROWNFIELD"]
...
flowchart TB
    ...
    C --> E["Mostrar contexto / + opções de recovery"]
    D --> F["Mostrar progresso / + opções de resume"]
```
...
flowchart TB
    ...
    Q2 -->|Não| PB["PATH B: / BROWNFIELD DISCOVERY"]
    Q2 -->|Sim| PC["PATH C: / ENHANCEMENT"]
    Q1 -->|Diretório vazio| PD["PATH D: / GREENFIELD"]
...
sequenceDiagram
    ...
    BOB->>U: "Encontrei N débitos. / [1] Resolver débitos / [2] Adicionar feature"
    U->>BOB: Escolha
...
flowchart TB
    ...
    subgraph PHASE3["Phase 3: Development"]
        L --> M["→ DEVELOPMENT CYCLE / (reutiliza WorkflowExecutor)"]
    end
...
flowchart LR
    ...
    RECENT --> CONTEXT["Accumulated Context / ≤ 8000 tokens"]
    MEDIUM --> CONTEXT
    OLD --> CONTEXT
```
