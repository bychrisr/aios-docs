...
flowchart TD
    subgraph PHASE_1["Fase 1: Story Creation"]
        ...
        SM_CREATE["@sm: Criar proxima story / Task: create-next-story"]
    end

    subgraph PHASE_2["Fase 2: Story Validation"]
        SM_CREATE --> PO_VALIDATE["@po: Validar story / Task: validate-next-story / 10 Checks"]
        PO_VALIDATE --> VALID_DECISION{Validacao OK?}
        VALID_DECISION -->|Nao| FEEDBACK_SM["Feedback para SM / Story retorna para ajustes"]
        FEEDBACK_SM --> SM_CREATE
        VALID_DECISION -->|Sim| DEV_IMPLEMENT
    end

    subgraph PHASE_3["Fase 3: Implementation"]
        DEV_IMPLEMENT["@dev: Implementar story / Task: dev-develop-story / + CodeRabbit Self-Healing"]
    end

    subgraph PHASE_4["Fase 4: QA Review"]
        DEV_IMPLEMENT --> QA_REVIEW["@qa: Review + Quality Gate / Task: qa-gate"]
        QA_REVIEW --> QA_DECISION{Quality Gate OK?}
        QA_DECISION -->|Nao| FEEDBACK_DEV["Feedback para Dev / Checklist de fixes"]
