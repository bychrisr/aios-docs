...
flowchart TB
    subgraph CLAUDE_NATIVE["Camada 1: Claude Code Nativo"]
        direction TB
        CLAUDE_MD_G["CLAUDE.md Global / ~/.claude/CLAUDE.md"]
        CLAUDE_MD_W["CLAUDE.md Workspace / Workspaces/.claude/CLAUDE.md"]
        CLAUDE_MD_P["CLAUDE.md Projeto / aios-core/.claude/CLAUDE.md"]
        RULES["Rules/*.md / 5 arquivos de regras"]
        AUTO_MEM["Auto Memory / ~/.claude/projects/.../memory/MEMORY.md / Primeiras 200 linhas no system prompt"]
        COMPOUND["Compound Analysis / memory/compound-analysis/ / 11 arquivos (9 conteudo + manifest + summary)"]
        AGENT_MEM["Agent Memory / .claude/agent-memory/{agent}/MEMORY.md / 6 agentes com memoria"]
        TRANSCRIPTS["Session Transcripts / ~/.claude/projects/.../{session}.jsonl"]
        SESSIONS_IDX["Sessions Index / sessions-index.json"]
    end

    subgraph AIOS_FRAMEWORK["Camada 2: AIOS Framework"]
        direction TB
        GOTCHAS["Gotchas Memory / .aios-core/core/memory/gotchas-memory.js"]
        CTX_SNAP["Context Snapshot / .aios-core/core/memory/context-snapshot.js"]
        FILE_EVO["File Evolution Tracker / .aios-core/core/memory/file-evolution-tracker.js"]
        TIMELINE["Timeline Manager / .aios-core/core/memory/timeline-manager.js"]
        CTX_LOADER["Session Context Loader / .aios-core/core/session/context-loader.js"]
        CTX_DETECT["Context Detector / .aios-core/core/session/context-detector.js"]
        SESS_STATE["Session State / .aios-core/core/orchestration/session-state.js"]
        HOOKS_GEM["Gemini Hooks / .aios-core/hooks/gemini/"]
        HOOKS_UNI["Unified Hooks / .aios-core/hooks/unified/"]
    end
...
flowchart TD
    ...
    subgraph LOAD["Carregamento Automatico — Ordem Fixa"]
        LOAD_GLOBAL["1. ~/.claude/CLAUDE.md / Instrucoes globais do usuario"]
        LOAD_WORKSPACE["2. Workspaces/.claude/CLAUDE.md / Instrucoes nivel workspace"]
        LOAD_PROJECT["3. aios-core/.claude/CLAUDE.md / Instrucoes nivel projeto"]
        LOAD_RULES["4. aios-core/.claude/rules/*.md / 5 arquivos de regras detalhadas"]
        LOAD_MEMORY["5. ~/.claude/projects/.../memory/MEMORY.md / Auto Memory (primeiras 200 linhas)"]
    end
...
flowchart TD
    subgraph READ_FLOW["Leitura (Automatica — Toda Sessao)"]
        ...
        R3 -->|"Sim"| R4["Trunca no limite / Linhas 201+ ignoradas"]
    ...
    subgraph WRITE_FLOW["Escrita (Durante Sessao — Decisao do Claude)"]
        W1["Claude encontra padrão, / erro ou insight"] --> W2{"Claude decide / salvar na memoria?"}
        W2 -->|"Sim"| W3["Usa Write/Edit tool / para atualizar MEMORY.md"]
        W2 -->|"Nao"| W4["Conhecimento perdido / ao fim da sessao"]
        W3 --> W5["Opcionalmente cria / arquivo topico em / memory/compound-analysis/"]
    end

    subgraph NO_FLUSH["FIM DA SESSAO"]
        E1([Sessao Fecha]) --> E2["Transcricao salva / como .jsonl"]
...
flowchart LR
    ...
    SQUAD_AGENTS["Agentes de Squad / (Claude Code Agents)"] -->|"frontmatter / memory: project"| AGENT_MEM

    NOTE["NOTA: Apenas agentes / definidos em .claude/agents/ / usam esta memoria. / Agentes AIOS (.aios-core/ / development/agents/) / NAO usam."]
...
flowchart TD
    subgraph GOTCHAS["gotchas-memory.js — Epic 9, Story 9.4"]
        ...
        G5[("Persiste em: / .aios/gotchas.json / .aios/gotchas.md / .aios/error-tracking.json")]
    end
    ...
    subgraph SNAPSHOTS["context-snapshot.js — Story 12.6"]
        ...
        S5[("Persiste em: / .aios/snapshots/{id}.json")]
    end
    ...
    subgraph FILE_EVO["file-evolution-tracker.js"]
        ...
        F4[("Persiste em: / .aios/file-evolution/ / evolution-index.json")]
    end
    ...
    subgraph TIMELINE_MGR["timeline-manager.js — Facade Unificada"]
        ...
        T4[("Persiste em: / .aios/timeline/ / unified-timeline.json")]
    end
...
flowchart TD
    ...
    subgraph CTX_LOADER["context-loader.js"]
        ...
        CL6["Retorna: tipo, msg, agente anterior, / comandos, workflow ativo"]
        CL7["updateSessionState()"]
        CL8["onTaskComplete()"]
        CL9[("Persiste em: / .aios/session-state.json / TTL: 1 hora")]
    ...
    subgraph SESS_STATE["session-state.js — Story 11.5"]
        ...
        SS7[("Persiste em: / docs/stories/ / .session-state.yaml / Sem TTL — persistente")]
...
sequenceDiagram
    ...
    Note over UAP: ANTES do pipeline, Claude Code / ja carregou: CLAUDE.md (3 niveis), / rules/*.md, MEMORY.md (200 linhas)
    ...
    Note over CC: NENHUMA memoria AIOS / e carregada automaticamente / no system prompt. / Apenas contexto de sessao / aparece no greeting.
```
...
flowchart TD
    ...
    subgraph SAVES["O Que e Salvo"]
        S1[".aios/session-state.json / Agente atual, comandos, workflow"]
        S2[".aios/error-tracking.json / Ocorrencia do erro"]
        S3[".aios/gotchas.json / Nova gotcha auto-capturada"]
        S4[".aios/gotchas.json / Gotcha manual adicionada"]
        S5["docs/stories/.session-state.yaml / Phase atualizada"]
        S6["docs/stories/.session-state.yaml / Story marcada done"]
        S7[".aios/snapshots/{id}.json / Contexto capturado"]
        S8[".aios/file-evolution/ / Evolucao registrada"]
    end
...
sequenceDiagram
    ...
    Note over Agent,FS: Inferencia de workflow: / 36 mapeamentos task→state / Ex: validate-next-story → / {workflow: story_development, / state: validated}
...
flowchart TD
    ...
    LOAD --> CHECK{"Estado existente / em .session-state.yaml?"}

    CHECK -->|"Nao"| FRESH["Sessao nova / Sem recovery necessario"]
    CHECK -->|"Sim"| DETECT["SessionState.detectCrash()"]

    DETECT --> THRESHOLD{"Ultima atividade / > 30 minutos atras?"}
    THRESHOLD -->|"Nao"| NORMAL["Sessao normal / Continua de onde parou"]
    THRESHOLD -->|"Sim"| LAST_ACTION{"Ultima acao foi / PAUSE, COMPLETED, / ou ABORT?"}

    LAST_ACTION -->|"Sim"| NORMAL_END["Encerramento normal / Nao e crash"]
    LAST_ACTION -->|"Nao"| CRASH["CRASH DETECTADO"]

    CRASH --> RESUME_MENU["Mostra opcoes de resume"]

    subgraph RESUME_OPTIONS["Opcoes de Recovery"]
        OPT1["CONTINUE / Retoma do ultimo estado"]
        OPT2["REVIEW / Mostra sumario de progresso"]
        OPT3["RESTART / Reinicia story atual"]
        OPT4["DISCARD / Descarta sessao"]
    end
...
flowchart TD
    subgraph ERROR_FLOW["Fluxo de Auto-Capture"]
        ...
        INCREMENT --> CHECK{"Contador >= 3? / (repeatThreshold)"}

        CHECK -->|"Nao"| SAVE_ET["Salva error-tracking.json / Aguarda proxima ocorrencia"]
        CHECK -->|"Sim"| WINDOW{"Dentro da janela / de 24 horas?"}

        WINDOW -->|"Nao"| RESET["Reset contador / Recomeça contagem"]
        WINDOW -->|"Sim"| AUTO_CAPTURE["_autoCaptureGotcha()"]

        AUTO_CAPTURE --> CREATE["Cria gotcha com: / - id: gotcha-{hash} / - source: auto_detected / - category: inferida / - severity: inferida"]
    ...
    subgraph INJECTION_FLOW["Fluxo de Injecao em Tasks"]
        ...
        QUERY --> MATCH["Filtra gotchas por: / - keywords do taskDesc / - arquivos relacionados / - categoria relevante"]
        MATCH --> FORMAT["formatForPrompt(gotchas)"]
        FORMAT --> INJECT["Injeta no contexto / da task como aviso"]
    end
...
flowchart TD
    subgraph CAPTURE["Capture — context-snapshot.js"]
        ...
        C2 --> C3["Captura estado git: / branch, commit, dirty files"]
        C3 --> C4["Registra: / storyId, agent, subtask, / working directory"]
        C4 --> C5["Salva .aios/snapshots/{id}.json"]
        C5 --> C6{"autoCleanup?"}
        C6 -->|"Sim"| C7["Remove snapshots: / - > 50 existentes / - > 7 dias de idade"]
    end

    subgraph RESTORE["Restore — Recuperacao"]
        R1(["snapshot.restore(id)"]) --> R2["Le .aios/snapshots/{id}.json"]
        R2 --> R3["Retorna contexto completo"]
        R3 --> R4["Agente pode retomar / de onde parou"]
    end
...
flowchart TD
    subgraph SOURCES["Fontes de Dados"]
        SRC1["FileEvolutionTracker / .aios/file-evolution/"]
        SRC2["ContextSnapshot / .aios/snapshots/"]
        SRC3["BuildStateManager / (opcional)"]
    ...
    subgraph TIMELINE["timeline-manager.js"]
        ...
        TM5["Aplica limites: / max 5000 entradas / max 90 dias"]
        TM6["autoSync (60s interval)"]
    end

    subgraph STORAGE["Storage"]
        ST1[(".aios/timeline/ / unified-timeline.json")]
    end
...
```mermaid
flowchart TD
    Q{"sessionStart no / Claude Code?"} -->|"NAO — claude: null"| GAP["NAO EXISTE evento / nativo no Claude Code / para session start"]
    GAP --> WORKAROUND["Workaround atual: / Pipeline de ativacao substitui / parcialmente via Tier 3 / (SessionContextLoader)"]

    Q2{"sessionEnd no / Claude Code?"} -->|"Mapeado para Stop"| STOP["Stop event existe / mas hook NAO esta / configurado no / .claude/settings.json"]
    STOP --> NOT_WIRED["session-end.js nunca / executa no Claude Code"]
