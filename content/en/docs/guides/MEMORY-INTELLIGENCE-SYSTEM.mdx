...
flowchart LR
    subgraph CORE["aios-core (Open Source, MIT)"]
        ...
        C5["Extension Points / (isProAvailable checks)"]
    end
    ...
    subgraph PRO["aios-pro (Privado, Submodule pro/)"]
        ...
        P8["pro/pro-config.yaml / memory.persistent: true"]
    end

    C5 -->|"isProAvailable()?"| CHECK[" "]
    CHECK -->|"Sim"| PRO
    CHECK -->|"Nao"| FALLBACK["Core funciona / como hoje"]
...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["aios-core (Open Source)"]
        direction TB
        EXT["Extension Points / in Pipeline"]
        HOOK_RUNNER[".aios-core/hooks/ / pro-hook-runner.js"]
        GOTCHAS["gotchas-memory.js / (standalone)"]
        CTX_LOADER["context-loader.js / (standalone)"]
    end

    subgraph PRO_BOUNDARY["aios-pro (Privado — pro/)"]
        subgraph CAPTURE["CAMADA 1: CAPTURE"]
            C1["session-digest.js / PreCompact + Stop"]
            C2["gotcha-capture.js / PostToolUseFailure"]
        end
        ...
        subgraph RETRIEVAL["CAMADA 3: RETRIEVAL"]
            R1["memory-retriever.js / Progressive Disclosure"]
            R2["memory-loader.js / Pipeline Integration"]
            R3["agent-memory-api.js / *recall, *remember"]
        end

        subgraph EVOLUTION["CAMADA 4: EVOLUTION"]
            E1["self-learner.js / Correction + Heuristic"]
            E2["rule-proposer.js / Auto-Evolution"]
        end
    end
...
```mermaid
flowchart LR
    SS([SessionStart]) --> PIPELINE["Pipeline carrega / CLAUDE.md + rules / + MEMORY.md (200 linhas)"]
    PIPELINE --> CHECK["isProAvailable()?"]
    CHECK -->|"NAO"| SKIP["Skip memory loading / (como hoje)"]
    SKIP --> GREETING["Greeting sem / memory stats"]

    PC([PreCompact]) --> HOOK["pro-hook-runner.js"]
    HOOK --> CHECK2["pro/ existe?"]
    CHECK2 -->|"NAO"| EXIT["exit(0) / Nada acontece"]
...
flowchart LR
    subgraph CORE["aios-core"]
        HR[".aios-core/hooks/ / pro-hook-runner.js"]
        UAP["unified-activation- / pipeline.js"]
        GM["gotchas-memory.js"]
        PD["bin/utils/ / pro-detector.js"]
    end

    subgraph PRO["aios-pro (pro/)"]
        SD["memory/ / session-digest.js"]
        GC["memory/ / gotcha-capture.js"]
        MI["memory/ / memory-index.js"]
        MR["memory/ / memory-retriever.js"]
        ML["memory/ / memory-loader.js"]
        SL["memory/ / self-learner.js"]
        RP["memory/ / rule-proposer.js"]
        API["memory/ / agent-memory-api.js"]
    end
...
flowchart TD
    ...
    subgraph PRO_CAPTURE["aios-pro: pro/memory/"]
        subgraph EXTRACTORS["Extractors"]
            EX1["Session Digest Extractor / Decisoes, erros, progresso"]
            EX2["Gotcha Extractor / Erro 3x → auto-gotcha"]
            EX3["Correction Extractor / Correcao → pattern"]
        end

        subgraph CLASSIFIERS["Classifiers"]
            CL1["Tier Classifier / session | daily | durable"]
            CL2["Sector Classifier / episodic | semantic | / procedural | reflective"]
            CL3["Scope Classifier / shared | agent-specific"]
        end

        EXTRACTORS --> CLASSIFIERS
        CLASSIFIERS --> PERSIST[("Persist to / .aios/memories/")]
    end
...
flowchart LR
    subgraph NATIVE["Claude Code Nativo (sempre funciona)"]
        N1[".claude/memory/MEMORY.md / 200 linhas max / Sempre no system prompt"]
        N2[".claude/agent-memory/[agent]/ / MEMORY.md por agente"]
    end

    subgraph PRO_MANAGED["pro/memory/ gerencia (quando disponivel)"]
        F1["Shared Memories / Markdown + frontmatter"]
        F2["Agent Memories / Privadas por agente"]
        F3["index.json / Search index"]
    end

    subgraph SYNC["Sync (pro apenas)"]
        S1["HOT memories / → MEMORY.md"]
        S2["Agent HOT / → agent MEMORY.md"]
    end
...
flowchart TD
    ...
    subgraph LEVEL1["Nivel 1: Index Scan (~50 tokens)"]
        L1A["Busca por metadata: / tags, tipo, agente, tier"]
    ...
    subgraph LEVEL2["Nivel 2: Context Retrieval (~200 tokens)"]
        L2A["Carrega sumario + evidence / de cada match"]
    end
...
flowchart TD
    ...
    subgraph PRO_EVOLUTION["pro/memory/ (Evolution)"]
        A1["self-learner.js / Extrai regra/heuristica"]
        A4["Confidence Scorer / Score = evidence * recency"]
        P1["rule-proposer.js / Gera proposta de mudanca"]
    end
    ...
    I3 --> A1
    A1 --> A4
    A4 -->|"confidence > 0.9 / + 5+ evidence"| P1
...
sequenceDiagram
    ...
    participant HR as pro-hook-runner.js / (aios-core)
    participant PD as pro-detector.js / (aios-core)
    participant SD as session-digest.js / (aios-pro)
    ...
    alt Pro NAO disponivel
        ...
        SD->>SD: Extract: decisions, errors, / corrections, progress
        SD->>SD: Classify: tier + sector + scope
        SD->>FS: Write memory files (.md)
        SD->>FS: Update index.json
        SD-->>CC: Inject summary: / "5 memories persisted. / Use *recall to access."
    end
...
```mermaid
sequenceDiagram
    participant UAP as UnifiedActivationPipeline / (aios-core)
    participant PD as pro-detector.js / (aios-core)
    participant ML as memory-loader.js / (aios-pro)
    participant MR as memory-retriever.js / (aios-pro)
    participant GB as GreetingBuilder / (aios-core)

    Note over UAP: ANTES: Claude Code ja carregou / CLAUDE.md, rules, MEMORY.md
