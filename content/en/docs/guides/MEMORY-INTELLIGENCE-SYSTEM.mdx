...
flowchart TB
    ...
    subgraph CORE_BOUNDARY["aiox-core (Open Source)"]
        ...
        HOOK_RUNNER[".aiox-core/hooks/ / pro-hook-runner.js"]
...
flowchart LR
    subgraph CORE["aiox-core"]
        HR[".aiox-core/hooks/ / pro-hook-runner.js"]
...
flowchart TD
    ...
    subgraph PRO_CAPTURE["aiox-pro: pro/memory/"]
        ...
        CLASSIFIERS --> PERSIST[("Persist to / .aiox/memories/")]
    end
...
sequenceDiagram
    ...
    participant HR as pro-hook-runner.js / (aiox-core)
    participant PD as pro-detector.js / (aiox-core)
    participant SD as session-digest.js / (aiox-pro)
...
```mermaid
sequenceDiagram
    participant UAP as UnifiedActivationPipeline / (aiox-core)
    participant PD as pro-detector.js / (aiox-core)
    participant ML as memory-loader.js / (aiox-pro)
    participant MR as memory-retriever.js / (aiox-pro)
    participant GB as GreetingBuilder / (aiox-core)
