---
title: "00 Shared Activation Pipeline"
---
...
flowchart TD
    ...
    J -->|yes| K[4. Context Section / intelligent narrative + recommendations]
    J -->|no| L[Skip context section]

    K --> M[sessionType === 'workflow' / AND lastCommands AND no contextSection?]
    L --> M

    M -->|yes| N[5. Workflow Suggestions / via WorkflowNavigator]
    M -->|no| O[Skip workflow suggestions]

    N --> P[6. Commands / filtered by visibility]
    O --> P
...
```mermaid
flowchart TD
    A[detectSessionType] --> B[conversationHistory / not null AND length > 0?]
