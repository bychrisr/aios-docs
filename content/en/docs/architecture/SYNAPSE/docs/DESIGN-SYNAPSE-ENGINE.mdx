...
flowchart LR
    ...
    INIT --> L0["L0: Constitution / 5ms | ~100tok"]
    L0 --> L1["L1: Global / 10ms | ~200tok"]
    L1 --> L2["L2: Agent / 15ms | ~300tok"]
    L2 --> L3["L3: Workflow / 15ms | ~200tok"]
    L3 --> L4["L4: Task / 20ms | ~300tok"]
    L4 --> L5["L5: Squad / 20ms | ~200tok"]
    L5 --> L6["L6: Keyword / 15ms | ~200tok"]
    L6 --> L7["L7: Star-Cmd / 5ms | ~200tok"]
...
```mermaid
flowchart TD
    NEW["First prompt"] --> CREATE["Create session JSON / (schema_version: 2.0)"]
    CREATE --> UPDATE["Per-prompt update: / prompt_count++ / last_activity / context bracket"]

    UPDATE --> AGENT{"@agent / command?"}
    AGENT -->|"Yes"| SET_AGENT["Set active_agent"]
    AGENT -->|"No"| WF

    SET_AGENT --> WF{"Workflow / detected?"}
    WF -->|"Yes"| SET_WF["Set active_workflow"]
    WF -->|"No"| TASK

    SET_WF --> TASK{"Task / active?"}
    TASK -->|"Yes"| SET_TASK["Set active_task"]
    TASK -->|"No"| CONT

    SET_TASK --> CONT["Continue"]

    CONT --> CHECK{"Stale? / (>24h inactive)"}
