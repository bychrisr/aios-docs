...
flowchart TB
    subgraph AIOS["SYNKRA AIOS"]
        ...
        subgraph CORE["Core: Task-First Engine"]
            TASK["Task Validada / (unidade primaria)"]
            DECISION{"Decision Tree: / Quem executa?"}
        end

        subgraph EXECUTORS["4 Tipos de Executores"]
            direction LR
            AGENT["Agent / IA Generativa"]
            WORKER["Worker / Script Deterministico"]
            CLONE["Clone / IA + Metodologia"]
            HUMAN["Humano / Julgamento Critico"]
        end

        subgraph WORKSPACE["Workspace Dinamico"]
            direction LR
            SQUADS["Squads / Equipes Modulares"]
            CONFIG["Config Personalizada / por Empresa"]
            COLLAB["Humanos + IA / Trabalhando Juntos"]
        end
    end
...
flowchart LR
    subgraph TRADITIONAL["Paradigma Tradicional"]
        ...
        NOTE1["Agentes decidem / o que executar"]
    end
    ...
    subgraph TASKFIRST["Task-First (AIOS)"]
        ...
        DEC1 --> E3["Clone"]
        DEC1 --> E4["Human"]
        NOTE2["Tasks definem o trabalho. / Executores sao intercambiaveis."]
    end
...
flowchart TB
    subgraph TASK_ANATOMY["Anatomia de uma Task"]
        ...
        EXEC["Executor Type: / Agent | Worker | Clone | Human"]
        AC["Acceptance Criteria"]
    end

    TASK_ANATOMY --> VALIDATE{"Task Validada?"}
    VALIDATE -->|"Sim"| EXECUTE["Execucao garantida / por qualquer executor"]
    VALIDATE -->|"Nao"| REJECT["Rejeitar: / Refinar task primeiro"]
...
flowchart TB
    subgraph EXECUTORS["Os 4 Executores do AIOS"]
        ...
        subgraph AGENT_BOX["AGENT — IA Generativa"]
            AGENT_DESC["Usa LLMs para tasks criativas, / analiticas ou generativas"]
            AGENT_EX["Ex: Analisar brief, gerar copy, / selecionar template, code review"]
        end

        subgraph WORKER_BOX["WORKER — Script Deterministico"]
            WORKER_DESC["Codigo/script que transforma / dados com logica predefinida"]
            WORKER_EX["Ex: Carregar config JSON, / validar HTML, exportar PNG, / calcular espacamento"]
        end

        subgraph CLONE_BOX["CLONE — IA + Heuristicas de Dominio"]
            CLONE_DESC["Agente IA aumentado com axiomas / e metodologia de uma pessoa especifica"]
            CLONE_EX["Ex: Validar Atomic Design (Brad Frost), / revisar copy (Alex Hormozi), / avaliar UX (Jakob Nielsen)"]
        end

        subgraph HUMAN_BOX["HUMAN — Operador Humano"]
            HUMAN_DESC["Pessoa real executando task / que requer julgamento subjetivo"]
            HUMAN_EX["Ex: Aprovar campanha $10k, / compliance legal, decisao estrategica, / edge cases criticos"]
        end
    end
...
```mermaid
flowchart TD
    START(["Nova Task para Executar"]) --> Q1{"Requer / Criatividade ou / Subjetividade?"}

    Q1 -->|"Nao"| Q2{"Algoritmo / Deterministico / Existe?"}
    Q1 -->|"Sim"| Q3{"Julgamento / Humano / Obrigatorio?"}

    Q2 -->|"Sim"| Q4{"Chama / API Externa?"}
    Q2 -->|"Nao"| Q3

    Q3 -->|"Sim"| Q5{"Decisao Critica / com Impacto / Legal/Financeiro?"}
    Q3 -->|"Nao"| Q6{"Metodologia / Especifica / Necessaria?"}

    Q4 -->|"Sim (nao-IA)"| WORKER1["WORKER / com API"]
    Q4 -->|"Nao"| WORKER2["WORKER / Script"]

    Q5 -->|"Sim"| HUMAN["HUMAN / Operador Humano"]
    Q5 -->|"Nao"| Q6

    Q6 -->|"Sim"| CLONE["CLONE / IA + Heuristicas"]
    Q6 -->|"Nao"| AGENT["AGENT / IA Generativa"]
...
flowchart LR
    subgraph SUBSTITUTION["Regras de Substituicao de Executores"]
        ...
        R1["Agent -> Worker / Quando task se torna / deterministica com dados"]
        R2["Human -> Agent / Quando automacao atinge / precisao aceitavel"]
        R3["Agent -> Clone / Quando metodologia especifica / aumenta qualidade"]
        R4["Clone -> Agent / Quando metodologia nao e / critica para o resultado"]
    end
...
flowchart LR
    subgraph H1["Agent + Worker (Fallback)"]
        A1["Agent tenta"] -->|"falha"| W1["Worker assume / (regra simples)"]
    end

    subgraph H2["Agent + Human (Review)"]
        A2["Agent processa / 100% do volume"] -->|"score < 80%"| HU1["Human revisa / casos criticos"]
    end

    subgraph H3["Clone + Agent (Validacao)"]
        A3["Agent cria / (criativo)"] --> C1["Clone valida / (metodologia)"]
        C1 -->|"invalido"| A3
    end
...
flowchart TB
    subgraph WS["Workspace Dinamico da Empresa"]
        ...
        subgraph FRAMEWORK[".aios-core/ — Framework (Imutavel)"]
            AGENTS_CORE["11 Agentes Core / dev, qa, architect, pm, / po, sm, analyst, devops, / data-engineer, ux, aios-master"]
            TASKS_CORE["45+ Tasks / Executaveis"]
            WORKFLOWS["Workflows / Orquestraveis"]
            CONSTITUTION["Constitution / Principios Inegociaveis"]
        end

        subgraph SQUADS["squads/ — Equipes Modulares"]
            SQ1["Squad Marketing / copywriter, designer, / traffic manager"]
            SQ2["Squad Data / data-engineer, analyst, / etl specialist"]
            SQ3["Squad Dominio / agentes especializados / no negocio da empresa"]
        end

        subgraph MINDS["Clones Cognitivos"]
            M1["Pedro Valerio / Processos & Sistemas"]
            M2["Brad Frost / Atomic Design"]
            M3["Alex Hormozi / Copywriting"]
            M4["Clone Customizado / Especialista da empresa"]
        end
        ...
        subgraph CONFIG["Configuracao por Empresa"]
            ...
            TS["Tech Stack"]
            ST["Source Tree"]
            IDE["IDE Rules / (Claude Code, Cursor, / Copilot, Gemini)"]
        end
    end
    ...
    HUMANS <-->|"Colaboracao / Human-in-the-Loop"| WS
...
flowchart LR
    ...
    WIZARD --> CHOOSE_SQUADS["Escolher Squads / (squads iniciais)"]
    WIZARD --> CHOOSE_IDE["Escolher IDEs / (Claude Code, Cursor...)"]
    WIZARD --> CHOOSE_CONFIG["Configurar / Preferencias"]

    CHOOSE_SQUADS --> SQ_OPS["HybridOps / (Pedro Valerio)"]
    CHOOSE_SQUADS --> SQ_CUSTOM["Squad Customizado / (dominio da empresa)"]
    CHOOSE_SQUADS --> SQ_MARKET["Marketplace / (squads prontos)"]
    ...
    SQ_OPS --> READY(["Workspace Pronto! / Humanos + IA / trabalhando juntos"])
    SQ_CUSTOM --> READY
    SQ_MARKET --> READY
    IDE1 --> READY
    IDE2 --> READY
...
flowchart TB
    subgraph COLLAB["Colaboracao Squads + Humanos"]
        ...
        subgraph PLANNING["Fase 1: Planejamento"]
            H_PM["PM Humano"] <-->|"refina requisitos"| AI_PM["@pm (Morgan) / Agent"]
            H_PM <-->|"valida arquitetura"| AI_ARCH["@architect (Aria) / Agent"]
            AI_PM --> PRD["PRD Completo"]
            AI_ARCH --> ARCH_DOC["Documento de / Arquitetura"]
        end

        subgraph EXECUTION["Fase 2: Desenvolvimento"]
            AI_SM["@sm (River) / Agent"] -->|"cria stories"| STORIES["Stories / Hiperdetalhadas"]
            STORIES --> AI_DEV["@dev (Dex) / Agent"]
            AI_DEV -->|"implementa"| CODE["Codigo"]
            CODE --> AI_QA["@qa (Quinn) / Agent"]
            H_DEV["Dev Humano"] <-->|"pair programming"| AI_DEV
            H_DEV <-->|"code review"| AI_QA
        end

        subgraph REVIEW["Fase 3: Quality Gates"]
            AI_QA -->|"PASS"| AI_DEVOPS["@devops (Gage) / Agent"]
            AI_QA -->|"FAIL"| AI_DEV
...
```mermaid
flowchart TD
    IDEA(["Ideia / Requisito"]) --> ANALYST["@analyst / Pesquisa & Analise"]
    ANALYST --> PM["@pm (Morgan) / Cria PRD"]
    PM --> ARCH["@architect (Aria) / Design Arquitetural"]

    ARCH --> SM["@sm (River) / Cria Stories"]
    SM --> PO{"@po (Pax) / Valida Story? / 10-point checklist"}

    PO -->|"GO >= 7/10"| DEV["@dev (Dex) / Implementa"]
    PO -->|"NO-GO < 7/10"| SM

    DEV --> CODERABBIT{"CodeRabbit / Self-Healing / max 2 iteracoes"}
    CODERABBIT -->|"CRITICAL"| DEV
    CODERABBIT -->|"OK"| QA{"@qa (Quinn) / Quality Gate / 7 checks"}

    QA -->|"PASS"| DEVOPS["@devops (Gage) / Push + PR"]
    QA -->|"CONCERNS"| DEVOPS
    QA -->|"FAIL"| DEV
    ...
    subgraph TASKS_FLOW["Cada passo e uma TASK"]
        direction LR
        NOTE["Cada caixa acima e uma Task. / Cada Task define inputs, outputs, / pre/post-conditions. / O executor (Agent) e apenas / o tipo padrao — poderia ser / Worker, Clone ou Human."]
    end
...
flowchart LR
    subgraph TRAD["Abordagem Tradicional"]
        direction TB
        T1["Humano faz TUDO"] --> T2["IA como chatbot / (pergunta-resposta)"]
        T2 --> T3["Sem estrutura / Sem quality gates"]
        T3 --> T4["Resultado: Inconsistente"]
    end

    subgraph TASKMASTER["Task Runners (Taskmaster etc)"]
        direction TB
        TM1["IA gera lista de tasks"] --> TM2["IA executa tasks / sequencialmente"]
        TM2 --> TM3["Sem validacao / Sem roles distintos"]
        TM3 --> TM4["Resultado: Fragil"]
    end

    subgraph AIOS_WAY["Synkra AIOS (Task-First)"]
        direction TB
        A1["Tasks validadas / com acceptance criteria"] --> A2["Decision tree seleciona / executor ideal"]
        A2 --> A3["Quality gates em / cada transicao"]
        A3 --> A4["Resultado: Confiavel / & Escalavel"]
    end
...
| Documento | Conteudo |
|-----------|----------|
| EXECUTOR-DECISION-TREE.md | Decision tree detalhado com exemplos e cost-benefit |
| [SYNAPSE-FLOWCHARTS.md](SYNAPSE/SYNAPSE-FLOWCHARTS.md) | 12 flowcharts do motor de contexto SYNAPSE |
| Constitution | Principios inegociaveis do framework |
| User Guide | Guia completo de uso do AIOS |
