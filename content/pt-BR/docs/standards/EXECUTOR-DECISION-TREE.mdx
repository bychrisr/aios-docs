...
```mermaid
graph TD
    Start[Task to Execute] --> Q1{RequiresCreativity orSubjectivity?}
    
    Q1 -->|No| Q2{DeterministicAlgorithmExists?}
    Q1 -->|Yes| Q3{HumanJudgmentRequired?}
    
    Q2 -->|Yes| Q4{ExternalAPI Call?}
    Q2 -->|No| Q3
    
    Q3 -->|Yes| Q5{CriticalDecisionwith Legal/FinancialImpact?}
    Q3 -->|No| Q6{SpecificMethodologyRequired?}
    
    Q4 -->|Yes| Worker[Workerwith API]
    Q4 -->|No| Worker2[WorkerScript]
    
    Q5 -->|Yes| Humano[Humano]
    Q5 -->|No| Q6
    
    Q6 -->|YesBrad Frost,Alex Hormozi,etc.| Clone[Clonewith Heuristics]
    Q6 -->|No| Agente[AgenteGeneric AI]
...
|----------|-------------------|-------|---------------|-------------|
| **Worker** | $ Low($0 - $0.001) | ⚡ Fast(- File I/O- Simple calculations- API calls (non-AI) |
| **Agente** | $$$$ High($0.001 - $0.01) | 🐌 Slow(3-10s) | ❌ No | - Creative tasks- Analysis- Generation- AI API calls |
| **Humano** | $$$ Medium($5 - $50) | 🐢 Very Slow(minutes-hours) | ❌ No | - Legal/financial decisions- Quality gates- Strategic decisions- Edge cases |
| **Clone** | $$$$ High($0.002 - $0.015) | 🐌 Slow(5-15s) | ⚠️ Partial | - Methodology validation- Expert review- Domain-specific tasks- Personality-driven execution |
...
| **Script Execution** | ⚠️ Limited(via tools) | ✅ Yes | ❌ No | ⚠️ Limited(via tools) |
| **File I/O** | ⚠️ Limited(via tools) | ✅ Yes | ❌ No | ⚠️ Limited(via tools) |
| **Database Access** | ⚠️ Limited(via tools) | ✅ Yes | ✅ Yes(via UI) | ⚠️ Limited(via tools) |
| **External APIs** | ✅ Yes(AI APIs) | ✅ Yes(Non-AI) | ❌ No | ✅ Yes(AI APIs) |
| **Creative Thinking** | ✅ Yes | ❌ No | ✅ Yes | ✅ Yes(guided by heuristics) |
| **Subjective Judgment** | ⚠️ Simulated | ❌ No | ✅ Yes | ✅ Yes(via axioms) |
