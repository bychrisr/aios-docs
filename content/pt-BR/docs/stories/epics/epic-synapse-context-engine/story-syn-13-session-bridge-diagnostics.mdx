---
title: "Story Syn 13 Session Bridge Diagnostics"
---
...
ENTAO um arquivo `.synapse/sessions/{sessionId}.json` e criado/atualizado
  E o campo `active_agent` contem `{ id: 'po', activated_at: , activation_quality: 'full'|'partial'|'fallback' }`
...
ENTAO o L2 faz match com `AGENT_PO_AGENT_TRIGGER=po` no manifest
  E as regras do domain `.synapse/agent-po` sao injetadas no ``
...
1. **Via star-command (L7)**: Quando detectado no prompt, o SYNAPSE injeta instrucoes nas `` para o Claude executar o diagnostico
