...
**Para** injetar regras contextuais `` automaticamente a cada prompt, substituindo o CARL hook como motor de contexto per-prompt.
...
3. Chama `engine.process(input)` para gerar `` XML
...
- **Hook Entry Point** em `.claude/hooks/synapse-engine.js`
  ...
  - Output: `{ hookSpecificOutput: { additionalContext: "..." } }` via `process.stdout`
...
- **Hook Registration** em `.claude/settings.local.json`
  ...
  - Command: `node "/synapse-engine.js"`
  - Type: `command`
